...
class Detalhes extends CI_Controller {
	...
	public function buscarDetalhes($produto_id){
			...
			<div class="row">
				...
				<div class="col-md-6 col-lg-6 col-sm-6 col-xs-6">
					...
					if($produto['usuario_id'] != $this->session->userdata('usuario_id')){
						...
						if(key_exists($produto_id, $produtos_interesses) && $produtos_interesses[$produto_id]):
						?>
							<a class="btn btn-danger"
							   href="<?php echo base_url('produtos/cancelarinteresse/') . $produto_id; ?>">
								Cancelar interesse
							</a>
						<?php
						elseif($this->doacao->buscarDoacaoPeloProduto($produto_id)):
						?>
							<p>Este produto já foi doado.</p>
						<?php
						else:
							?>
							<a class="btn btn-default"
							   href="<?php echo base_url('produtos/mostrarinteresse/') . $produto_id; ?>">
								Mostrar interesse
							</a>
						<?php
						endif;
		...
		?>
			<p>Este produto não está mais disponível. Ele pode ter sido excluído ou doado.</p>
		<?php
		endif;
	}
	...
	public function filtrar($interesse = null){
		...
		if($categoria == null && $estado == null && $cidade == null){
			if($interesse == 1){
				...
				$dados['produtos'] = $this->produto->buscarProdutosDisponiveis($usuario_id);
			}
		}
		else{
